...
public class DefaultMessageImpl extends AbstractMessage {
	 ...
	 * @return Hashtable { nation_key : { property_key : property_value } } 
	 * ex) { "ko" : { "ok" : "성공", "fail" : "실패" }, "en" : { "ok" : "ok", "fail" : "fail" } }
	 * @desc WAS기 기동되는 시점에 호출되어 처리된다. 다국어 처리 Message를 로드하여 Hashtable로 리턴한다. 
	 ...
	 * ex) { "ko" : { "ok" : "성공", "fail" : "실패" }, 
	 *       "en" : { "ok" : "ok", "fail" : "fail" } }
	 * @desc HTTP Request 호출시 기동시점에 로드한 메시지가 없는 경우 최초에 호출된다. 다국어 처리 Message를 로드하여 Hashtable로 리턴한다.
	 */
